...
import { findApp, formatBytes, resolveAssetUrl, resolveDownloadUrl } from "@/lib/catalog";
...
export default async function AppDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article>
      ...
      {app.rating || app.age_rating || app.category ? (
            ...
            {app.releases.map((release) => (
              <article className="release" key={release.version}>
                <div className="release__copy">
                  ...
                  <span>{formatBytes(release.size)} · {release.created_at}</span>
